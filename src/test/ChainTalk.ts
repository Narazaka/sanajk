...
const autoTalks = new SanaJK.AutoTalks<DefaultState>([
  chain(
    "1",
    "2",
    chain("3-1", "3-2"),
    ...
    endchain(
      ...
      chain("5-3-1", "5-3-2"),
      "5-4",
      auto(chain("5-5-1a", "5-5-2a"), chain("5-5-1b", "5-5-2b")),
      "5-6",
      endchain(
        "5-7-1",
        "5-7-2",
        auto(
          endchain("5-7-3-1a", "5-7-3-2a"),
          endchain("5-7-3-1b", "5-7-3-2b")
        ),
        "5-7-4"
      ),
      "5-8"
  ...
  describe("autoTalkMiddleware", () => {
    describe("ChainTalk", () => {
      it("works", async () => {
        ...
        const autoTalkTrigger = new SanaJK.AutoTalkTrigger({
          autoStart: false,
        });
        const ctx = {
        ...
        for (let i = 0; i < 2000; ++i) {
          ...
          switch (random1) {
            case "5-5-1a":
              assert(autoTalkChooser.choose(ctx).exec(ctx) === "5-5-2a");
              break;
            case "5-5-1b":
              assert(autoTalkChooser.choose(ctx).exec(ctx) === "5-5-2b");
              break;
            default:
              assert.fail();
          }
          ...
          switch (random2) {
            case "5-7-3-1a":
              assert(autoTalkChooser.choose(ctx).exec(ctx) === "5-7-3-2a");
              break;
            case "5-7-3-1b":
              assert(autoTalkChooser.choose(ctx).exec(ctx) === "5-7-3-2b");
              break;
            default:
              assert.fail();
          }
